...
$mini = $_POST['duree_mini'];
    if (empty($_POST['duree_mini'])){
        header('location: form_recherche.php');
        exit();
    }
    $mini = filter_var($mini, FILTER_VALIDATE_INT);
    $mini = htmlentities($mini);
$maxi = $_POST['duree_maxi'];
    if (empty($_POST['duree_maxi'])){
        header('location: form_recherche.php');
        exit();
    }
    $maxi = filter_var($maxi, FILTER_VALIDATE_INT);
    $maxi = htmlentities($maxi);
$real = $_POST['real'];
    if (empty($_POST['real'])){
        header('location: form_recherche.php');
        exit();
    }
    $real = filter_var($real, FILTER_SANITIZE_STRING);
